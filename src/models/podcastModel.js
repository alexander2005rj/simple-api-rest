...
function findIndexBy(id) {
    const index = podcastData.findIndex(
        (podcastData) => podcastData.id === parseInt(id),
    );
    return index;
}
...
function createPodcast(jsonBody, response) {
    ...
    if (index >= 0)
        return response
            .status(400)
            .json({ error: `podcast_id ${newPodcast.id} already exists` });
    podcastData.push(newPodcast);
    response.status(201).json({ created: newPodcast, status: "succeeded" });
}

function searchPodcasts(response) {
    response.status(200).json(podcastData);
}

function searchPodcastById(id, response) {
    const index = findIndexBy(id);
    if (index < 0)
        return response.status(404).json({ error: `podcast_id ${id} not found` });
...
function partialUpdatePodcast(jsonBody, id, response) {
    ...
    if (jsonBody.podcastName)
        podcastData[index].podcastName = partialData.podcastName;
